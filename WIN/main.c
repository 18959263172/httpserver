...
TCHAR szTitle[200];					// 标题栏文本
TCHAR szWindowClass[200];			// 主窗口类名
...
static char_t	*demoWeb = T("www");		/* Root web directory */
...
static int __wodda_webs_cgi_handler(webs_t wp, char_t *urlPrefix, char_t *webDir, int arg,
	char_t *url, char_t *path, char_t* query);
int hx_strnicmp(char *path,const char *file,int len);
 ...
 */
	if (windowsInit(hinstance,cmd_show)<0) 
	{
		return FALSE;
	}
/*
...
void GetTermTime(webs_t wp, char_t *path, char_t *query)
{
	websWrite(wp, T(" 取消 "));
	websFooter(wp);
	websDone(wp, 200);
 ...
 */
/*	..websAspDefine(T("GetTermTime"), aspGetTermTime);*/
#ifdef USER_MANAGEMENT_SUPPORT
	umOpen();
 ...
 */
	...
	websUrlHandlerDefine(T("/cgi-bin"), NULL, 0, __wodda_webs_cgi_handler, 0);
 ...
 */
	...
	websFormDefine(T("GetTermTime"), GetTermTime);
/*
...
static int windowsInit(HINSTANCE hinstance,int cmd_show)
{
	HWND hWnd;
	WNDCLASSEX wcex;
	wcex.cbSize = sizeof(WNDCLASSEX);
	wcex.style			= CS_HREDRAW | CS_VREDRAW;
	wcex.lpfnWndProc	= websWindProc;
	wcex.cbClsExtra		= 0;
	wcex.cbWndExtra		= 0;
	...
	wcex.hIconSm		= LoadIcon(wcex.hInstance, MAKEINTRESOURCE(IDI_TEST));

	LoadString(hinstance, IDC_TEST1, szTitle, 100);
	LoadString(hinstance, IDC_TEST1, szWindowClass, 100);
...
static long CALLBACK websAboutProc(HWND hwndDlg, unsigned int msg,unsigned int wp,long lp)
{
	long lResult;
	HWND hwnd;
...
static int __wodda_webs_cgi_query_phrase(char* query, char* list[])
{
	char *ptr = NULL;
	int i = 0;
	int nums = 0;
	...
	for(i = 0; i < nums * 2; i += 2)
	{
		...
		list[i + 0] = strtok(list[i], "=");
		list[i + 1] = strtok(NULL, "=");
		//        fprintf(stderr, "(%s) = (%s)\r\n",list[i], list[i+1]);
	}
	list[i] = NULL;
	return 0;
	...
	char_t *url, char_t *path, char_t* query)
{
	...
	status=(struct status_s *)malloc(sizeof(struct status_s));

	//fprintf(stderr, "__wodda_webs_cgi_handler: urlPrefix(%s),webDir(%s),arg(%d),url(%s),  path(%s),query(%s)\r\n",
	//urlPrefix,webDir,arg,url,path,query);

	memset(list, 0, sizeof(char*) * 256);

	__wodda_webs_cgi_query_phrase(query, list);
	...
	if(hx_strnicmp(path, "/behemoth/playback.cgi", strlen("/behemoth/playback.cgi")) == 0)
	{
		//wodda_cgi_playback(wp, method, list);
	}
	else if(hx_strnicmp(path, "/behemoth/capture.cgi", strlen("/behemoth/capture.cgi")) == 0)
	{
		//wodda_cgi_capture(wp, method, list);
	} 
	else if (hx_strnicmp(path, "/behemoth/settime.cgi", strlen("/behemoth/settime.cgi")) == 0) 
	{
		//wodda_cgi_settime(wp, method, list);        
	} else if (hx_strnicmp(path, "/behemoth/gettime.cgi", strlen("/behemoth/gettime.cgi")) == 0) {
		//wodda_cgi_gettime(wp, method, list);        
	}
	else if (hx_strnicmp(path, "/behemoth/getdiskid.cgi", strlen("/behemoth/getdiskid.cgi")) == 0) {
		//wodda_cgi_getdiskid(wp, method, list);
	} 
	else 
	{
		//wodda_cgi_default(wp, method, list);
	}
    if (hx_strnicmp(path, "/cgi-bin/cgitest.cgi", strlen("/cgi-bin/cgitest.cgi")) == 0) 
	{
		 process_cgi( wp,status);
		//wodda_cgi_getdiskid(wp, method, list);
	}
	else 
	{
...
int process_cgi(void* wp, struct status_s *status)
{
	char *state[3] = {"Idle", "Playback", "Capture"};
	char storage[128];

	snprintf(storage, 128, "%0.2lf GfffB", status->size / 1024.0);
	websWrite(wp, T("{\"State\":\"%s\",\"Storage\":\"%s\"}"), 
		state[status->state%3], storage);
